...
# output files
fn_fasta <- paste0(window_outdir, params$prefix, ".fa")
```
...
if (!file.exists(fn_fasta) || params$redo) {
  ...
  if (params$round_len < 0) {
    s_ln <- round(s_ln, params$round_len)
    ...
    write.table(paste0("Updated number of sites: ", s_ln, "bp"),
                file=fn_log, quote=F, row.names=F, col.names=F, append=T)
...
if (length(params$window_size) > 0){
  for (w in params$window_size) {
    if (s_ln %% w != 0) {
      msg <- paste0("Warning: ", s_ln, " is not divisible by ", w, ". Skipped.")
      log4r::warn(fn_logger, msg)
      next
    }
...
sfactors <- getFactors(s_ln)
sfactors <- sfactors[-wsize]
